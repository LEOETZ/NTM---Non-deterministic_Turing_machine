...
struct hash_state_node* find(int key_state,char key_read);
        ...
        */
        ...
        if (ban_index(state_of_run, section_run[position_run])!=baned_index){
            if(pickup_from_stack==false){
                ...
                matched_at_hash=find(state_of_run, section_run[position_run]);
        ...
        if(there_is_road==true || pickup_from_stack==true){
            ...
            /*
                        //测试
                        printf("state=%d  matched_at_hash->read=%c  matched_at_hash->next_state=%d  section_run[position_run]=%c\n",
            ...
            */
            ...
            if(match_nd_next!=NULL){                           //有分叉路
                ...
                */
                ...
                backup_run=calloc(strlen(section_run)+3, sizeof(char));
                if (backup_run==NULL) {
                    fprintf(stderr, "Out of space！！！！！For backup_run");
                }
                memcpy(backup_run, section_run, strlen(section_run));  //to do backup
                stack=push(state_of_run,position_run,match_nd_next, backup_run,stack,pri_max);
            }
